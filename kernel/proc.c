...
// Initialize a thread structure for a process.
...
// Exit the current thread, freeing its resources.
void
exitthread()
{
  struct proc *p = myproc();
...
// Join a thread with the given id.
int
...
// Initialize a thread for the process.
struct thread *
...
// Schedule a thread for the process.
int
thread_schd(struct proc *p)
{
  ...
  for (int i = 0; i < NTHREAD; i++, t++) {
    ...
    if (t->state == THREAD_RUNNABLE) {
      ...
      break;
               ...
               ticks0 - t->sleep_tick0 >= t->sleep_n) {
      next = t;
      break;
               }
  }
  ...
  if (p->current_thread != next) {
    ...
    struct thread *old = p->current_thread;
    p->current_thread = next;

    if (old->trapframe)
      *old->trapframe = *p->trapframe;
